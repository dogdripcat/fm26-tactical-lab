...
class DesktopUiV11Tests(unittest.TestCase):
    ...
    def test_compact_rebuild_uses_recognition_and_dual_role_cells(self):
        ...
        self.assertIn('id="playing-approach"', html)
        self.assertNotIn('id="playing-approach" disabled', html)
        self.assertLess(html.index('style-control'), html.index('approach-control'))
        self.assertNotIn('id="pitch-legend"', html)
        self.assertIn("const knownFormation=phase=>recognizeFormation(active(phase),data.registry,data.formationFamilies).formation", script)
